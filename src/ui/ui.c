...
static void init_styles(void)
{
    ...
    lv_style_set_text_font(&style_status_text, &lv_font_montserrat_20);
...
static void build_channel_screen(uint8_t channel)
{
    ...
    lv_obj_set_flex_flow(reset_row, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_flex_align(reset_row, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_top(reset_row, 4, LV_PART_MAIN);
    lv_obj_set_style_pad_row(reset_row, 2, LV_PART_MAIN);

    sensor_status_label[channel] = lv_label_create(reset_row);
    lv_label_set_text(sensor_status_label[channel], "Sensor: Connected");
    lv_obj_add_style(sensor_status_label[channel], &style_status_text, LV_PART_MAIN);
    lv_obj_set_width(sensor_status_label[channel], lv_pct(100));
    lv_obj_set_style_pad_top(sensor_status_label[channel], 0, LV_PART_MAIN);

    lv_obj_t *reset_button = lv_btn_create(reset_row);
    lv_obj_set_width(reset_button, lv_pct(82));
    lv_obj_set_height(reset_button, 62);
    ...
    lv_obj_set_style_translate_y(reset_button, 10, LV_PART_MAIN);
...
{
    const char *status_text = connected ? "Sensor: Connected" : "Sensor: Not Connected";

    for (uint8_t ch = 0; ch < CHANNEL_COUNT; ch++) {
        if (sensor_status_label[ch] != NULL) {
            lv_label_set_text(sensor_status_label[ch], status_text);
        }
    }
}
